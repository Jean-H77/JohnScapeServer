...
public class OverloadPotionTask extends Task {
	...
	public void execute() {
		...
		if(timer == 600 || timer == 598 || timer == 596 || timer == 594 || timer == 592) {
			...
			player.dealDamage(new Hit(null, 100, Hitmask.RED, CombatIcon.NONE));
		}
		...
		if(player.getOverloadPotionTimer() <= 0 || player.getLocation() == Location.DUEL_ARENA || player.getLocation() == Location.WILDERNESS && player.getRights() != PlayerRights.DEVELOPER) {
