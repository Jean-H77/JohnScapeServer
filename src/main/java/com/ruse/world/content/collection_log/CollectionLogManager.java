...
public class CollectionLogManager {
    ...
    public void showLog(Log log) {
       if(log == currentLog) return;
        ...
        List<PlayerLog.LogEntry> entries = playerLog.getEntries();
        int[] logsItemsIds = log.getRequiredItemIds();
        Item[] logItemsToDisplay = new Item[logsItemsIds.length];

        for(int i = 0; i < logsItemsIds.length; i++) {
            if(entries.size() > i) {
                for (PlayerLog.LogEntry entry : entries) {
                    if ((logsItemsIds[i] == entry.getItemId() && entry.getItemAmount() > 0)) {
                        logItemsToDisplay[i] = new Item(logsItemsIds[i], entry.getItemAmount());
                    }
                }
            } else {
               logItemsToDisplay[i] = new Item(logsItemsIds[i], 0);
            }
        }

        p.getPacketSender().sendScrollMax(40445, Math.max(146, (logItemsToDisplay.length / 6 * 15)));

        PacketBuilder builder = new PacketBuilder(39, Packet.PacketType.SHORT);

        putItemData(builder, logItemsToDisplay, LOG_ITEM_CONTAINER_ID);
    ...
    public String getNameText(Log log, PlayerLog plog) {
        int logAmount = log.getRequiredItemIds().length;
        int pLogAmount = plog.getEntries().size();
        return logAmount == pLogAmount ? "@gre@"+log.getName():
                        pLogAmount > 0 ? "@yel@" + pLogAmount + "/" + log.getName() :
                                log.getName();
    }
    ...
    private void handleEntry(String logName, Item item, PlayerLog playerLog) {
        ...
        if(optionalLog.isPresent()) {
            ...
            p.getPacketSender().sendMessage("@red@Code shouldn't reach here");
        }
    }
}
