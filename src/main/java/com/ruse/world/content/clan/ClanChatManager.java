...
public class ClanChatManager {
	...
	public static void addDefault() {
		File file = new File(FILE_DIRECTORY + GameSettings.RSPS_NAME + ".json");
		if (!file.exists()) {
			ClanChat clan = new ClanChat(0);
			clan.setName(GameSettings.RSPS_NAME);
			clan.setOwnerName(GameSettings.DEFAULT_CLAN_OWNER);
			clan.getRankedNames().put(GameSettings.DEFAULT_CLAN_OWNER, ClanChatRank.OWNER);
			writeFile(clan);
			System.out.println("Created clan chat '" + GameSettings.RSPS_NAME + "' with owner '" + GameSettings.DEFAULT_CLAN_OWNER + "'.");
		}
	}
	...
	public static void join(Player player, String channel) {
		...
		for (ClanChat clan : clans) {
			if (clan != null) {
				if(clan.getOwnerName().toLowerCase().equals(channel) || clan.getName().toLowerCase().equals(channel)) {
					join(player, clan);
					return;
				}
			}
	...
	public static void updateList(ClanChat clan) {
		Collections.sort(clan.getMembers(), (o1, o2) -> {
	...
	public static void leave(Player player, boolean kicked) {
		...
		if(player.getCurrentDungeon() != null && player.isInDungeon()) {
	...
	public static void delete(Player player) {
		try {
			...
			if (clan.getName().equalsIgnoreCase(GameSettings.RSPS_NAME)) {
				player.getPacketSender().sendMessage("Cannot delete.");
				return;
			}

			File file = new File(FILE_DIRECTORY + clan.getName().replaceAll(" ", "\\ "));

			for(int i = 0; i < clan.getDungeons().size(); i++) {
				if(clan.getDungeons().get(i) != null) {
					clan.getDungeons().get(i).end();
				}
			}
	...
	public static void toggleLootShare(Player player) {
		...
		if(clan.getLastAction().elapsed(5000) || player.getRights().isStaff()) {
			if (clan.getName().equalsIgnoreCase("JohnScape")) {
