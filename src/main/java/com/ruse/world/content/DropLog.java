...
public class DropLog {
	...
	public static class DropLogEntry {
		...
		public DropLogEntry(int item, int amount, int npcId) {
			this.item = item;
			this.amount = amount;
			this.npcId = npcId;
			this.rareDrop = ItemDefinition.forId(item).getValue() > 200000 || NpcItemDropping.ItemDropAnnouncer.announce(item);
		}
