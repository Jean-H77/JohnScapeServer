...
import com.ruse.world.content.combat.strategy.impl.bosses.Scorpia;
...
public final class CombatFactory {
	 ...
	 */
	public static boolean fullVeracs(CharacterEntity entity) {
	 ...
	 */
	public static boolean fullDharoks(CharacterEntity entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Dharok the Wretched")
	 ...
	 */
	public static boolean fullKarils(CharacterEntity entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Karil the Tainted")
	 ...
	 */
	public static boolean fullAhrims(CharacterEntity entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Ahrim the Blighted")
	 ...
	 */
	public static boolean fullTorags(CharacterEntity entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Torag the Corrupted")
	 ...
	 */
	public static boolean fullGuthans(CharacterEntity entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Guthan the Infested")
	...
	/**
	 * Attempts to poison the argued {@link CharacterEntity} with the argued
	 ...
	 */
	public static void poisonEntity(CharacterEntity entity, Optional<PoisonType> poisonType) {
	 ...
	 *            the poison type that this entity is being inflicted with.
	 */
	public static void poisonEntity(CharacterEntity entity, PoisonType poisonType) {
	 ...
	 */
	public static Hit getHit(CharacterEntity entity, CharacterEntity victim, CombatType type, int maxHit) {
		switch (type) {
		case MELEE:
			return new Hit(entity, maxHit == -1 ? Misc.inclusiveRandom(1, DesolaceFormulas.getMeleeMaxHit(entity, victim)) :  Misc.inclusiveRandom(maxHit), Hitmask.RED, CombatIcon.MELEE);
		case RANGED:
			return new Hit(entity, maxHit == -1 ? Misc.inclusiveRandom(1, CombatFactory.getRangedMaxHit(entity, victim)) : Misc.inclusiveRandom(maxHit), Hitmask.RED, CombatIcon.RANGED);
		case MAGIC:
			return new Hit(entity, maxHit == -1 ? Misc.inclusiveRandom(1, DesolaceFormulas.getMagicMaxHit(entity)) : Misc.inclusiveRandom(maxHit), Hitmask.RED, CombatIcon.MAGIC);
		case DRAGON_FIRE:
			return new Hit(entity, maxHit == -1 ? Misc.inclusiveRandom(0, CombatFactory.getDragonFireMaxHit(entity, victim)) : Misc.inclusiveRandom(maxHit), Hitmask.RED, CombatIcon.MAGIC);
		default:
			throw new IllegalArgumentException("Invalid combat type: " + type);
		}
	}

	public static Hit getMaxHit(CharacterEntity entity, CharacterEntity victim, CombatType type) {
		switch (type) {
			case MELEE:
				return new Hit(entity, DesolaceFormulas.getMeleeMaxHit(entity, victim), Hitmask.RED, CombatIcon.MELEE);
			case RANGED:
				return new Hit(entity,CombatFactory.getRangedMaxHit(entity, victim), Hitmask.RED, CombatIcon.RANGED);
			case MAGIC:
				return new Hit(entity,DesolaceFormulas.getMagicMaxHit(entity), Hitmask.RED, CombatIcon.MAGIC);
			case DRAGON_FIRE:
				return new Hit(entity,CombatFactory.getDragonFireMaxHit(entity, victim), Hitmask.RED, CombatIcon.MAGIC);
			default:
				throw new IllegalArgumentException("Invalid combat type: " + type);
		}
	}
	...
	public static boolean rollAccuracy(CharacterEntity attacker, CharacterEntity victim, CombatType type) {
	...
	/**
	 * Calculates the maximum melee hit for the argued {@link CharacterEntity} without
	 ...
	 */
	@SuppressWarnings("incomplete-switch")
	public static int calculateMaxMeleeHit(CharacterEntity entity, CharacterEntity victim) {
		int maxHit = 0;
	...
	/**
	 * Calculates the maximum ranged hit for the argued {@link CharacterEntity} without
	 * taking the victim into consideration.
	 * 
	 * @param entity
	 *            the entity to calculate the maximum hit for.
	 ...
	 */
	@SuppressWarnings("incomplete-switch")
	public static int getRangedMaxHit(CharacterEntity entity, CharacterEntity victim) {
		int maxHit = 0;
		if (entity.isNpc()) {
			NPC npc = (NPC) entity;
			maxHit = npc.getDefinition().getMaxHit();
		...
		int rangedStrength = ((int) player.getBonusManager().getOtherBonus()[BonusManager.RANGED_STRENGTH]);
	...
	public static int getDragonFireMaxHit(CharacterEntity e, CharacterEntity v) {
		int baseMax = 250;
	 ...
	 */
	public static boolean checkHook(CharacterEntity entity, CharacterEntity victim) {
		...
		if(victim.isNpc() && entity.isPlayer()) {
			...
			if(Nex.nexMob(npc.getId())) {
				...
				if(entity.getCombatBuilder().getStrategy().getCombatType(entity) == CombatType.MELEE) {
		...
		if (entity.getCombatBuilder().getLastAttacker() != null && !Locations.inMulti(entity) && entity.getCombatBuilder().isBeingAttacked() && !victim.equals(entity.getCombatBuilder().getLastAttacker())) {
			if (entity.isPlayer())
		...
		if(!(entity.isNpc() && ((NPC)entity).isSummoningNpc())) {
			...
			if (victim.getCombatBuilder().getLastAttacker() != null && !Locations.inMulti(entity) && victim.getCombatBuilder().isBeingAttacked() && !victim.getCombatBuilder().getLastAttacker().equals(entity)) {
	...
	public static boolean checkAttackDistance(CharacterEntity a, CharacterEntity b) {
		...
		if(/*a.isPlayer() && (strategy.getCombatType(a) == CombatType.RANGED || strategy.getCombatType(a) == CombatType.MAGIC && ((Player)a).getCastSpell() != null && !(((Player)a).getCastSpell() instanceof CombatAncientSpell)) || */a.isNpc() && strategy.getCombatType(a) == CombatType.MELEE) {
	...
	protected static void handleArmorEffects(CharacterEntity attacker, CharacterEntity target, int damage, CombatType combatType) {
		if(attacker.getConstitution() > 0 && damage > 0) {
			if(target != null && target.isPlayer()) {
				...
				if(t2.getEquipment().getItems()[Equipment.RING_SLOT].getId() == 2550) {
					...
					attacker.dealDamage(new Hit(target, recDamage, Hitmask.RED, CombatIcon.DEFLECT));
	 ...
	 *            the total amount of damage dealt.
	 */
	protected static void handlePrayerEffects(CharacterEntity attacker, CharacterEntity target, int damage, CombatType combatType) {
			...
			if (attacker.isPlayer()) {
				...
				if (PrayerHandler.isActivated(victim, PrayerHandler.RETRIBUTION) && victim.getConstitution() < 1) {
					...
					if (p.getPosition().isWithinDistance(victim.getPosition(),CombatFactory.RETRIBUTION_RADIUS)) {
						p.dealDamage(new Hit(target, Misc.inclusiveRandom(CombatFactory.MAXIMUM_RETRIBUTION_DAMAGE), Hitmask.RED, CombatIcon.DEFLECT));
					}
					...
					TaskManager.submit(new Task(1, victim, false) {
						...
						public void execute() {
							if(attacker == null || attacker.getConstitution() <= 0) {
								...
								attacker.dealDamage(new Hit(victim, toDeflect, Hitmask.RED, CombatIcon.DEFLECT));
							stop();
						}
					});
				}
	...
	protected static void handleSpellEffects(CharacterEntity attacker, CharacterEntity target, int damage, CombatType combatType) {
		if(damage <= 0)
			return;
		if(target.isPlayer()) {
			...
			if(t.hasVengeance()) {
				...
				if(attacker.getConstitution() < returnDamage)
					returnDamage = attacker.getConstitution();
				attacker.dealDamage(new Hit(target, returnDamage, Hitmask.RED, CombatIcon.MAGIC));
			}
		}
	...
	public static void handleDragonFireShield(final Player player, final CharacterEntity target) {
		...
		TaskManager.submit(new Task(1, player, false) {
			...
			public void execute() {
				...
				case 3:
					...
					break;
				case 4:
					Hit h = new Hit(player, 50 + (Misc.getRandom(20)*10), Hitmask.RED, CombatIcon.MAGIC);
					target.dealDamage(h);
