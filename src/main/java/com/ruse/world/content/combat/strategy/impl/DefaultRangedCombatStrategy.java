...
/**
 * The default combat strategy assigned to an {@link CharacterEntity} during a ranged
...
public class DefaultRangedCombatStrategy implements CombatStrategy {
	...
	public CombatContainer attack(CharacterEntity entity, CharacterEntity victim) {
		if (entity.isNpc()) {
			...
			case 6252:
				...
				break;
				case 8777:
					ammo = AmmunitionData.HAND_CANNON_SHOT;
					break;
				case 351:
					ammo = AmmunitionData.ZARYTE_AMMO;
					break;
			}
	...
	private boolean checkAmmo(Player player) {
		...
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 10156) {
			return true;
		}
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 9307 ) {
			return true;
		}
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 15241 ) {
			return true;
		}
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 895 ) {
			return true;
		}
		if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 895 ) {
			return true;
	...
	public static void decrementAmmo(Player player, Position pos) {
		...
		if (player.getEquipment().get(slot).getAmount() == 0) {
			if (player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 22010
				|| player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 10156 || player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 9307 || player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 15241 || player.getEquipment().get(Equipment.WEAPON_SLOT).getId() == 895) {
				return;
			} else {
	...
	private static final int getModifiedDamage(Player player, CharacterEntity target, CombatContainer container) {
	...
	//	}
		...
		case 9242:
			...
			player.dealDamage(new Hit(null, priceDamage, Hitmask.RED, CombatIcon.RANGED));
			return dmg2;
		case 9243:
