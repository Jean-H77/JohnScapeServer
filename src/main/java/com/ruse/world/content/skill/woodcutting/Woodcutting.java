package com.ruse.world.content.skill.woodcutting;
					...
					if (player.getSkillManager().getCurrentLevel(Skill.WOODCUTTING) >= (isEvilTree ? t2.getWoodcuttingLevel() : t.getReq())) {
						...
						player.setCurrentTask(new Task(1, player, false) {
							...
							public void execute() {
								...
								if (++cycle % 4 == 0 && player.getSkillManager().isSuccess(Skill.WOODCUTTING, isEvilTree ? t2.getWoodcuttingLevel() : t.getReward(), h.getRequiredLevel())) {
								...
								//	player.getPacketSender().sendMessage("Random: " + cutDownRandom);
									if (!isEvilTree && (!t.isMulti() || (player.getSkillManager().skillCape(Skill.WOODCUTTING) && cutDownRandom >= 88)
											|| (!player.getSkillManager().skillCape(Skill.WOODCUTTING) && cutDownRandom >= 82))) {//82
