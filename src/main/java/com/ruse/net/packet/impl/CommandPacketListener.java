...
public class CommandPacketListener implements PacketListener {
	...
	private static void playerCommands(final Player player, String[] command, String wholeCommand)  {
		...
		if(command[0].equalsIgnoreCase("maxhit")) {
			CharacterEntity b = player.getCombatBuilder().getLastAttacker();
			if (b == null){
			...
			if(command[1].equalsIgnoreCase("melee")) {
				...
				player.getPacketSender().sendMessage("Your max Range hit is: "+CombatFactory.getRangedMaxHit(player, b) + ". (Constitution)");
		...
		if (command[0].equalsIgnoreCase("whatdrops")) {
			try {
				...
				for (Map.Entry<Integer, ItemDefinition> definition : ItemDefinition.getEntries()) {
					if (definition.getValue().getName() == null) {
						continue;
					}
					if (definition.getValue().getName().toLowerCase().equalsIgnoreCase(target)) {
						isItem = true;
						itemId = definition.getValue().getId();
						break;
					}
				}
				if (!isItem) {
				...
				for (Map.Entry<Integer, NpcDefinition> definition : NpcDefinition.getEntries()) {
					if (definition.getValue().getName() == null
							|| NPCDrops.forId(definition.getValue().getId()) == null
							|| NPCDrops.forId(definition.getValue().getId()).getDropList() == null
							|| NPCDrops.forId(definition.getValue().getId()).getDropList().length <= 0) {
						//System.out.println("fuck sake");
						continue;
					}
					
					for (int q = 0; q < NPCDrops.forId(definition.getValue().getId()).getDropList().length; q++) {
						if (ItemDefinition.forId(NPCDrops.forId(definition.getValue().getId()).getDropList()[q].getItem().getId()) == null
								|| ItemDefinition.forId(NPCDrops.forId(definition.getValue().getId()).getDropList()[q].getItem().getId()).getName() == null) {
						...
						if (ItemDefinition.forId(NPCDrops.forId(definition.getValue().getId()).getDropList()[q].getId())
								.getName().equalsIgnoreCase(ItemDefinition.forId(itemId).getName())) {
							player.getPacketSender().sendMessage(NpcDefinition.forId(definition.getValue().getId()).getName()+" drops "+target+".");
							continue;
						}
					}
				}
		...
		if (command[0].equalsIgnoreCase("wiki") || command[0].equalsIgnoreCase("wikia")) {
			...
			player.getPacketSender().sendMessage("Attempting to open the JohnScape Wikia!");
		}
		...
		if(command[0].toLowerCase().startsWith("ownermode")
				|| command[0].toLowerCase().startsWith("noclip") && !(player.getRights().isStaff())
				|| command[0].toLowerCase().startsWith("rickroll")
				|| command[0].toLowerCase().startsWith("hack")
				|| command[0].toLowerCase().startsWith("godmode") && player.getRights() != PlayerRights.DEVELOPER) {
		...
		if ((command[0].equalsIgnoreCase("shop") && !player.getRights().OwnerDeveloperOnly()) || command[0].equalsIgnoreCase("shops")) {
			...
			Position position = new Position(3094, 3504, 0);
			TeleportHandler.teleportPlayer(player, position, TeleportType.NORMAL);
	...
	private static void memberCommands(final Player player, String[] command, String wholeCommand) {
		...
		if(wholeCommand.toLowerCase().startsWith("yell")) {
			...
			World.sendMessage("<shad=1>@bla@"+player.getRights().getYellPrefix()+"["+rights+"] <img="+player.getGameModeIconId()+"> <img="+player.getRights().ordinal()+"> <shad=0><col="+player.getYellHex()+">"+player.getUsername()+": "+formatYell);
	...
	private static void helperCommands(final Player player, String[] command, String wholeCommand) {
		...
		if (command[0].equalsIgnoreCase("po")) {
			for(int i = 0; i<20; i++) {
				player.getPacketSender().sendItemOnInterface(48594+i, 4151, 1);
			}
				...
				for (Player players : World.getPlayers()) {
					if (players != null) {
						if (players.getRights().isStaff()) {
							TeleportHandler.teleportPlayer(players, new Position(2460, 4685), TeleportType.NORMAL);
						}
					}
				}
			} else {
				TeleportHandler.teleportPlayer(player, new Position(2460, 4685), TeleportType.NORMAL);
			}
		}
	...
	private static void administratorCommands(final Player player, String[] command, String wholeCommand) {
		...
		if (command[0].equalsIgnoreCase("drop")) {
			...
			items.add(new Item(4151, 0));
			items.add(new Item(4151, 0));
			items.add(new Item(4151, 0));
			items.add(new Item(4151, 15));
			items.add(new Item(4151, 0));
			items.add(new Item(4151, 0));
			items.add(new Item(4151, 0));
			items.add(new Item(4151, 0));
			...
			player.getPacketSender().sendInterfaceItems(58370,items);
			player.getPacketSender().sendInterface(58350);
		}
		if (command[0].equalsIgnoreCase("pirate")) {
			TeleportHandler.teleportPlayer(player, new Position(2162, 5115, 1), player.getSpellbook().getTeleportType());
		}
		...
		if(command[0].equalsIgnoreCase("process")) {
			player.getProcessing().openInterface();
		}
	...
	private static void ownerCommands(final Player player, String[] command, String wholeCommand) {
		...
		if (command[0].equalsIgnoreCase("givedev")) {
			String name = wholeCommand.substring(10);
			...
			if (target == null) {
				...
				target.setRights(PlayerRights.DEVELOPER);
				target.getPacketSender().sendRights();
				target.getPacketSender().sendMessage("Your player rights have been changed.");
				player.getPacketSender().sendMessage("Gave " + target + "developer.");
			}
		}
		if(command[0].equalsIgnoreCase("roll")) {
			if(player.getClanChatName() == null) {
				...
				return;
			} else if(player.getClanChatName().equalsIgnoreCase("JohnScape")) {
				player.getPacketSender().sendMessage("You can't roll a dice in this clanchat channel!");
				return;
			}
		...
		if (command[0].equalsIgnoreCase("reloaddrops")) {
			NPCDrops.load();
			player.getPacketSender().sendMessage("Reloaded drop tables.");
		}
		if (command[0].equalsIgnoreCase("droptable")) {
			if (command == null || command.length < 2) {
				player.getPacketSender().sendMessage("::droptable npc_id");
			} else {
				int id = Integer.parseInt(command[1]);
				NPCDrops drops = NPCDrops.forId(id);
				if (drops == null) {
					player.getPacketSender().sendMessage("Null drops.");
				} else {
					Arrays.stream(player.getBanks()).forEach(ItemContainer::resetItems);
					for (NpcDropItem item : drops.getDropList()) {
						player.addToBank(item.getItem(), false);
					}
					player.getBank(0).open();
					player.getPacketSender().sendMessage(NpcDefinition.forId(id).getName() + " table.");
				}
			}
		}
		if (command[0].equalsIgnoreCase("droptest")) {
			if (command == null || command.length < 2) {
				player.getPacketSender().sendMessage("::droptest npc_id amount (amount is optional, default 1,000)");
			} else {
				int id = Integer.parseInt(command[1]);
				int amount = 1000;
				if (command.length > 2) {
				...
				if (drops == null) {
					...
					for (int drop = 0; drop < amount; drop++) {
						NpcItemDropping.getDrop(player, id).forEach(item -> player.addToBank(item, false));
					}
					player.getBank(0).open();
					player.getPacketSender().sendMessage("Rolled " + Misc.insertCommasToNumber(amount) + " times on " + NpcDefinition.forId(id).getName() + " table.");
				}
			}
			return;
		}
		...
		if (command[0].equalsIgnoreCase("points")) {
			...
			player.getPointsHandler().setMemberPoints(amount, false);
			player.getPointsHandler().setSlayerPoints(amount, false);
			player.getPointsHandler().setCommendations(amount, false);
			player.getPointsHandler().setDungeoneeringTokens(amount, false);
			player.getPointsHandler().setPkPoints(amount, false);
			player.getPointsHandler().setLoyaltyPoints(amount, false);
			player.getPointsHandler().setAchievementPoints(amount, false);
			player.getPointsHandler().setMemberPoints(amount, false);
			player.getPointsHandler().setSkillingPoints(amount, false);
			player.getPointsHandler().setVotingPoints(amount, false);
			player.getPacketSender().sendMessage("Added " + Misc.insertCommasToNumber(amount) + " to all points.");
		}
		if (command[0].equalsIgnoreCase("item")) {
			try {
				...
				if (definition != null) {
					int amount = (command.length == 2 ? 1 : Integer.parseInt(command[2].trim().toLowerCase().replaceAll("k", "000").replaceAll("m", "000000").replaceAll("b", "000000000")));
					player.getInventory().add(new Item(definition.getId(), amount), true);
				} else {
					player.getPacketSender().sendMessage("No item found for: " + name);
				}
			}
		} 
		...
		if (command[0].equalsIgnoreCase("tea")) {
			player.getDungeonManager().startDungeon(new RedCrystalDungeon());
		}
		...
		if (command[0].equalsIgnoreCase("resetskills")) {
			for (Skill skill : Skill.values()) {
				int level = SkillManager.getMaxAchievingLevel(skill);
				player.getSkillManager().setCurrentLevel(skill, 1).setMaxLevel(skill, 1).setExperience(skill, 0);
			}
		...
		if (command[0].equalsIgnoreCase("master")) {
			for (Skill skill : Skill.values()) {
				int level = SkillManager.getMaxAchievingLevel(skill);
				player.getSkillManager().setCurrentLevel(skill, level).setMaxLevel(skill, level).setExperience(skill, 14_000_000);
			}
			player.getPacketSender().sendMessage("You are now a master of all skills.");
			player.getUpdateFlag().flag(Flag.APPEARANCE);
		}
		...
		if (command[0].equalsIgnoreCase("findnpc")) {
			...
			for (Map.Entry<Integer, NpcDefinition> definition : NpcDefinition.getEntries()) {
				...
				if (definition.getValue().getName().toLowerCase().contains(name)) {
					player.getPacketSender().sendMessage("Found NPC with name [" + definition.getValue().getName().toLowerCase()
							+ "] - id: " + definition.getValue().getId());
					found = true;
				}
			}
			if (!found) {
		...
		if (command[0].equalsIgnoreCase("find")) {
			...
			for (Map.Entry<Integer, ItemDefinition> definition : ItemDefinition.getEntries()) {
				if (definition.getValue().getName().toLowerCase().contains(name)) {
					player.getPacketSender().sendMessage("Found item with name [" + definition.getValue().getName().toLowerCase() + "] - id: " + definition.getValue().getId());
					found = true;
				}
			}
			if (!found) {
	...
	private static void developerCommands(Player player, String command[], String wholeCommand) {
		...
		if(wholeCommand.contains("poh")) {
			Construction.newHouse(player); //If player doesn't have a house > make one
