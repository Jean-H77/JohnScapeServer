	...
	public void handleMessage(Player player, Packet packet) {
		...
		if (item.getId() != -1 && item.getAmount() >= 1) {
			if(item.tradeable()) {
				...
				destroyItemInterface(player, item, itemSlot);
			}
		}
	}

	public static void destroyItemInterface(Player player, Item item, int slot) {//Destroy item created by Remco
		player.setDestroyItemSlot(slot);
