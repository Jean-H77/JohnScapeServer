...
import com.ruse.world.content.skill.hunter.JarData;
import com.ruse.world.content.skill.hunter.PuroPuro;
import com.ruse.world.content.skill.hunter.SnareTrap;
...
public class ItemActionPacketListener implements PacketListener {
	...
	private static void firstAction(final Player player, Packet packet) {
		...
		switch(itemId) {
			...
			case 31538:
				...
				player.setDialogueActionId(365);
				break;
			case 2946:
	...
	public static void secondAction(Player player, Packet packet) {
		...
		if (SummoningData.isPouch(player, itemId, 2))
			return;
		switch(itemId) {
		case Items.RING_OF_SUFFERING_R:
			RecoilRing.checkCharges(player, slot);
			return;
		case 6500:
