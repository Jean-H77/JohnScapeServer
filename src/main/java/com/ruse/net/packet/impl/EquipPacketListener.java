		...
		case Inventory.INTERFACE_ID:
			...
			if (slot >= 0 && slot <= 28) {
				...
				if (item != null && id == item.getId()) {
					for (SkillLevel level : item.getDefinition().getRequirements()) {
						...
						if (level.getLevel() > player.getSkillManager().getMaxLevel(level.getSkill())) {
							player.getPacketSender().sendMessage("You need "
									+ Misc.anOrA(level.getSkill().getName()) + " "
									+ Misc.formatText(level.getSkill().getName())
									+ " level of at least "
									+ level.getLevel()
									+ " to wear this.");
							return;
					...
					if (equipmentSlot == -1 || item.getDefinition().isNoted()) {
						player.getPacketSender().sendMessage("This item cannot be equipped!");
						return;
					}
