...
public class PacketSender {
	...
	/**
	 * Send osrs region ids
	 * @param regions the region ids
	 * @return the {@link PacketSender}
	 */
	public PacketSender sendOsrsRegions(int...regions) {
		PacketBuilder out = new PacketBuilder(2, PacketType.SHORT);
		out.putShort(regions.length);
		Arrays.stream(regions).forEach(out::putInt);
	...
	public PacketSender sendScrollMax(int interfaceId, int scrollMax) {
	...
	public PacketSender sendTabs() {
		...
		sendTabInterface(GameSettings.QUESTS_TAB, 49426);
	...
	public PacketSender sendItemContainer(NpcDropItem[] items, int interfaceId) {
		...
		for (NpcDropItem item : items) {
			...
			if (item.getItem().getAmount() > 254) {
				out.put((byte)255);
				out.putInt(item.getItem().getAmount(), ByteOrder.INVERSE_MIDDLE);
			} else {
				out.put(item.getItem().getAmount());
			}
			out.putShort(item.getItem().getId() + 1, ValueType.A, ByteOrder.LITTLE);
		}
		player.getSession().queueMessage(out);
		return this;
	}
	...
	public PacketSender sendCombatBoxData(CharacterEntity character) {
		...
		if(character.isPlayer()) {
			...
			boolean sendList = npc.getDefaultConstitution() >= 2500 && Locations.inMulti(npc);
	...
	public PacketSender constructMapRegion(Palette palette) {
		...
		bldr.initializeAccess(PacketBuilder.AccessType.BYTE);
		player.getSession().queueMessage(bldr);
		return this;
	}
	...
	public PacketSender sendRichPresenceDetails(String details) {
	...
	public PacketSender sendRichPresenceState(String state) {
