...
public class Locations {
	...
	public static boolean inMulti(CharacterEntity gc) {
		...
		if(gc.getLocation() == Location.WILDERNESS) {
			...
			//senga
			if(x >= 2127 && x <= 2190 && y >= 4999 && y <= 5031) {
				return true;
			}
		}
	...
	public enum Location { 
			...
			public void process(Player player) {
				...
				if (safe) {
					...
					*/
					...
					player.dealDamage(new Hit(null, Misc.getRandom(15)*10, Hitmask.DARK_PURPLE, CombatIcon.CANNON));
		...
		SISTER_SENGA_AREA(new int[]{2158,2128,2158,2128}, new int[]{4999, 5030,5030,4999}, true, true, false, false, false, false) {
		},
		...
		STARTER_ZONE(new int[]{3071, 3008, 3008,3071}, new int[]{2816, 2879, 2816,2879}, false, false, true, true, true, false) {
		},
		...
		public static boolean ignoreFollowDistance(CharacterEntity character) {
			Location location = character.getLocation();
			return location == Location.FIGHT_CAVES || location == Location.RECIPE_FOR_DISASTER || location == Location.NOMAD;
		}
	}

	public static void process(CharacterEntity gc) {
