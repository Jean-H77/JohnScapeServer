...
public class Shop extends ItemContainer {
	...
	public void checkValue(Player player, int slot, boolean sellingItem) {
		...
		if(getCurrency().getId() != -1) {
			...
			if(id == TOKKUL_EXCHANGE_STORE || id == STARDUST_EXCHANGE_STORE || id == ENERGY_FRAGMENT_STORE || id == BOOTY_SHOP || id == AGILITY_TICKET_STORE || id == GRAVEYARD_STORE || id == BARROWS_STORE || id == MEMBERS_STORE_I ||id == MEMBERS_STORE_II || id == MEMBERS_STORE_III) {
	 ...
	 */
	@Override
	public Shop switchItem(ItemContainer to, Item item, int fromSlot, boolean sort, boolean refresh) {
		...
		if (!shopSellsItem(item))
			return this;
		if(getItems()[fromSlot].getAmount() <= 1 && id != GENERAL_STORE) {
			player.getPacketSender().sendMessage("The shop has run out of stock for this item.");
			return this;
		}
		if (item.getAmount() > getItems()[fromSlot].getAmount())
			item.setAmount(getItems()[fromSlot].getAmount());
		int amountBuying = item.getAmount();
		if(amountBuying == getItems()[fromSlot].getAmount() && id != GENERAL_STORE) {
				amountBuying = getItems()[fromSlot].getAmount()-1;
		...
		if(id == SKILLCAPE_STORE_1 || id == SKILLCAPE_STORE_2 || id == SKILLCAPE_STORE_3) {
			for (SkillLevel level : item.getDefinition().getRequirements()) {
				int req = level.getLevel();
				if((level.getSkill() == Skill.CONSTITUTION || level.getSkill() == Skill.PRAYER) && req == 99)
					req *= 10;
				if(req > player.getSkillManager().getMaxLevel(level.getSkill())) {
					player.getPacketSender().sendMessage("You need to have at least level 99 in "+Misc.formatText(level.getSkill().toString().toLowerCase())+" to buy this item.");
					return this;
				}
			}
		} else if (id == 83) {
		...
		for (int i = amountBuying; i > 0; i--) {
			...
			if(!item.getDefinition().isStackable()) {
				if(playerCurrencyAmount >= value && hasInventorySpace(player, item, getCurrency().getId(), value)) {
					...
					super.switchItem(to, new Item(item.getId(), 1), fromSlot, false, false);
				...
				if(playerCurrencyAmount >= value && hasInventorySpace(player, item, getCurrency().getId(), value)) {
					...
					super.switchItem(to, new Item(item.getId(), canBeBought), fromSlot, false, false);
					playerCurrencyAmount -= value;
					break;
				} else {
					break;
	...
	public static class ShopManager {
		...
		public static void load() {
			File directory = new File(GameSettings.DEFINITION_DIRECTORY + "shops");
			if (!directory.exists()) {
				throw new IllegalStateException("No shop shop directory.");
			}

			for (File file : Objects.requireNonNull(directory.listFiles())) {
				getLoader(file.getPath()).loadObject();
			}
		}

		public static JsonLoader getLoader(String file) {
		...
		public static Object[] getCustomShopData(int shop, int item) {
			if(shop == BOOTY_SHOP) {
				switch (item) {
					case 7110:
					case 7116:
						return new Object[]{225, "Gold fragments"};
					case 7114:
					case 2651:
						return new Object[]{100, "Gold fragments"};
					case 2997:
						return new Object[]{250, "Gold fragments"};
					case 42355:
						return new Object[]{400, "Gold fragments"};
					case 21580:
					case 31295:
					case 31325:
						return new Object[]{300, "Gold fragments"};
					case 31538:
						return new Object[]{500, "Gold fragments"};
				}
	...
	private static final int BOOTY_SHOP = 99;
