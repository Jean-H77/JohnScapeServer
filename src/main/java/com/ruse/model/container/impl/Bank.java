...
public class Bank extends ItemContainer {
	...
	@Override
	public Bank switchItem(ItemContainer to, Item item, int fromSlot, boolean sort, boolean refresh) {
		if(!getPlayer().isBanking() || getPlayer().getInterfaceId() != 5292
				|| to instanceof Inventory && !(getPlayer().getBank(getPlayer().getCurrentBankTab()).contains(item.getId())
				|| getPlayer().getBankSearchingAttribtues().getSearchedBank() != null && getPlayer().getBankSearchingAttribtues().getSearchedBank().contains(item.getId()))) {
			getPlayer().getPacketSender().sendClientRightClickRemoval();
			return this;
		}
		...
		if(getPlayer().getBankSearchingAttribtues().isSearchingBank()
			...
			if (getItems()[fromSlot].getId() != item.getId() || !contains(item.getId()))
				return this;
	...
	public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
		...
		for (Item it : from.getValidItems()) {
			...
			Item toBank = it;
			if (toBank.getDefinition().isNoted()) {
				toBank = new Item(Item.getUnNoted(toBank.getId()), it.getAmount());
			}

			int tab = getTabToDepositItem(p, toBank);
	...
	public static int getTabToDepositItem(Player player, Item item) {
		if (item.getAttributes().hasAttributes()) {
			return player.getCurrentBankTab();
		} else {
			return getTabForItem(player, item.getId());
		}
	}
